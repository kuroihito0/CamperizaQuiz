...
const Incorrect = () => {
    const [technologyQuestionsData, setTechnologyQuestionsData] = useState<any[]>([]);
    ...
    useEffect(() => {
        const fetchIncorrectQuestions = async () => {
            try {
                const incorrectQuestionsSnapshot = await getDocs(query(collection(db, 'InCorrect')));
                ...
                if (incorrectQuestionsData.length > 0) {
                    const technologyQuestionsSnapshot = await getDocs(
                        query(
                            ...
                            ...(filteredIncorrectQuestionIds.length > 0
                                ? [where('問題ID', 'in', filteredIncorrectQuestionIds)]
                                : [])
                        )
                    );
                    ...
                    const technologyQuestionsData = technologyQuestionsSnapshot.docs.map((doc) => ({
                        ...
                        questionCount:
                            incorrectQuestionsData.find((d) => d.incorrectQuestionId === doc.data()['問題ID'])?.count || 0,
                    }));
                    console.log(incorrectQuestionsData);
